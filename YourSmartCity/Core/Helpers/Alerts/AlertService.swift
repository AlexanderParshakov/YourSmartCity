...
final class AlertService {
    ...
    static func showScanningInvalid(shortText: String) {
        // Generate top floating entry and set some properties
        var attributes = EKAttributes.centerFloat
        attributes.entryBackground = .gradient(gradient: .init(
            colors: [EKColor(Constants.Colors.UIGradientSets.deepSpace[1]), EKColor(Constants.Colors.UIGradientSets.deepSpace[1])],
            startPoint: .zero,
        ...
        let title = EKProperty.LabelContent(text: shortText, style: .init(font: .boldSystemFont(ofSize: 21), color: .white))
